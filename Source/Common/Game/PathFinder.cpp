...
{
    //Safety check that the state is Idle, if it isn't then a path is either
	//already found and in the process of being found
...
void PathFinder::update(double aDelta)
{
	...
	{
		//Saftey check that there is actually something in the open list
		...
		//Add the node to the closedf list and remove it from the open list
		...
		//If we got here that means we haven't reached our destination. We need to
		//get the adjacent tiles from the node and add them to the open list
		std::vector<Tile*> adjacentTiles;
			...
			//Check the tile right, is it walkable?
			addAdjacentTile(adjacentTiles, currentNode->getTile(), -1, 0);

			//Check the tile left, is it walkable?
			addAdjacentTile(adjacentTiles, currentNode->getTile(), 1, 0);

			
		//Cylce through the adjacent tiles that are walkable
